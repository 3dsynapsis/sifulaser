...
//
// The preview draws through the same nest() the exporter writes, so the layout
// on screen is the layout in the file rather than a second renderer that can
// drift apart from it.
//
...
// layer each. A single-line face engraves as open polylines - the exact path
// the head follows, nothing to fill. A real typeface engraves as the closed
// outline of every letter, which has to be scanned solid: send those to a Line
// operation and the machine traces round each letter and leaves it hollow. The
// medallion outline is a line, a name in Great Vibes is a fill, and one rehal
// carries both.
//
...
export const LAYERS = {
  ...
  engraveFill: { id: 'engrave-fill', color: '#000000', label: 'Ukir (isi)' },
  engrave: { id: 'engrave-line', color: '#0000ff', label: 'Ukir (garis)' },
};
...
/**
 * Shelf-pack the pieces left to right, wrapping at the sheet width.
 *
 * Deliberately not clever. A tighter pack would shuffle the pieces around, and
 * the one thing that matters when you pick five parts off a bed is being able
 * to tell which is which - so the decorated panel stays first and the two
 * cheeks stay next to each other, however much sheet that costs.
 */
export function nest(panels, { gap = 8, sheetWidth = 900 } = {}) {
  const placed = [];
  let x = 0;
  let y = 0;
  let rowH = 0;
...
export function toSvg(panels, opts = {}) {
  const o = {
    title: 'Rehal', gap: 8, sheetWidth: 900, strokeWidth: 0.1, ...opts,
  };
...
export function toPdf(panels, opts = {}) {
  ...
  ops.push('0 0 0 rg');
  // Every ring is closed into ONE path, and that path is filled once at the
  // end. Non-zero matches the winding the glyph data already carries, so a
  // counter stays a counter - but only across subpaths of the same path.
  // Filling each ring as it is drawn gives the counter nothing to cancel
  // against and paints the middle of every A, B and R solid black.
  let filling = false;
  ...
  const objects = [
    ...
    `<< /Title (${pdfString(o.title)}) /Creator (SifuLaser Rehal Generator) >>`,
  ];
