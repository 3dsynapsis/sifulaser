...
export const TOOLS = {
  ...
  converter: { name: 'Converter File', path: '/converter/' },
  rehal: { name: 'Rehal', path: '/rehal/' },
};

/** The tool this copy of the gallery is running inside. */
export const TOOL = 'rehal';
 ...
 * three entries called Rehal tanpa nama stacked on each other - which is what
...
export async function saveCurrent(store, { name, asNew = false } = {}) {
  const title = String(name || store.name() || 'Rehal tanpa nama').trim();
...
export function saveDialogBody(h, store, close, rerender) {
  ...
  const commit = (asNew) => async () => {
    ...
    failure = null;
    try {
      const name = input.value.trim() || 'Rehal tanpa nama';
      store.rename(name);
  ...
  return h('div', { class: 'dlg-body' },
    ...
    droppedExtra
      ? h('p', { class: 'warn-line' },
        'Sebahagian data reka bentuk ini terlalu besar untuk disimpan, jadi '
        + 'hanya tetapannya yang disimpan. Eksport fail potong untuk '
...
export function filesDialogBody(h, store, close, rerender) {
  ...
  return h('div', { class: 'dlg-body' },
    ...
    h('p', { class: 'hint' },
      'Yang disimpan ialah tetapannya - corak, nama, font, ukuran, sudut - bukan '
