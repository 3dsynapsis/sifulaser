...
export function strokesToOutline(strokes, opts = {}) {
  ...
  // Growing the letters is free here, and that is the whole point of a distance
  // field: the field already says how far every point is from the nearest bit of
  // ink, so fattening the shape by a millimetre is a millimetre off the level
  // that gets traced. No offsetting, no self-intersection to clean up. It is
  // what makes a hairline script cuttable, and it is also how neighbouring
  // letters and overlapping lines are made to actually touch.
  const grow = Math.max(0, opts.grow ?? 0);
  const span = Math.max(x1 - x0 + (r + grow) * 2, y1 - y0 + (r + grow) * 2, 1);
  let cell = opts.cell ?? Math.min(0.4, Math.max(0.06, span / 900));
  // A metre-wide sign at a fifth of a millimetre is a hundred million samples.
  // Coarsening is the right answer rather than refusing: the grid is a tracing
  // resolution, and on something that big a coarser one is still far finer than
  // the beam. Refusing would just leave the user with nothing.
  if (!opts.cell) {
    const w = x1 - x0 + (r + grow) * 2;
    const h = y1 - y0 + (r + grow) * 2;
    const budget = 8e6;
    const need = (w / cell + 8) * (h / cell + 8);
    if (need > budget) cell *= Math.sqrt(need / budget);
  }
  const pad = r + grow + cell * 3;
  const ox = x0 - pad;
  const oy = y0 - pad;
  ...
  // Every band below has to clear `grow` as well as its own radius: the field
  // is only sampled near the ink, and a contour cannot be traced out into
  // cells that were never measured.
  const stamp = (paths, radius) => {
    const band = radius + grow + cell * 3;
  ...
  for (const rings of groups) {
    ...
    const gb = grow + cell * 3;
    const i0 = clampI(Math.floor(iOf(gx0 - gb)));
    const i1 = clampI(Math.ceil(iOf(gx1 + gb)));
    const j0 = clampJ(Math.floor(jOf(gy0 - gb)));
    const j1 = clampJ(Math.ceil(jOf(gy1 + gb)));
    const gw = i1 - i0 + 1;
    const gh = j1 - j0 + 1;
    ...
    for (const ring of rings) {
      for (let k = 0, m = ring.length - 2; k < ring.length; m = k, k += 2) {
        ...
        const bx = ring[k];
        const by = ring[k + 1];
        const eb2 = grow + cell * 3;
        const ea = clampI(Math.floor(iOf(Math.min(ax, bx) - eb2)));
        const eb = clampI(Math.ceil(iOf(Math.max(ax, bx) + eb2)));
        const ec = clampJ(Math.floor(jOf(Math.min(ay, by) - eb2)));
        const ed = clampJ(Math.ceil(jOf(Math.max(ay, by) + eb2)));
  ...
  for (const [sx0, sy0, sx1, sy1] of solids) {
    const sb = grow + cell * 3;
    const i0 = clampI(Math.floor(iOf(sx0 - sb)));
    const i1 = clampI(Math.ceil(iOf(sx1 + sb)));
    const j0 = clampJ(Math.floor(jOf(sy0 - sb)));
    const j1 = clampJ(Math.ceil(jOf(sy1 + sb)));
    for (let j = j0; j <= j1; j++) {
      const py = oy + j * cell;
      const row = j * nx;
      for (let i = i0; i <= i1; i++) {
  ...
  if (grow > 0) for (let i = 0; i < v.length; i++) v[i] -= grow;
  ...
  const minArea = opts.minArea
    ?? (strokes.length ? Math.max(0.5, r * r * 0.6) : Math.max(0.3, grow * grow));
