...
//
// A keychain is one piece, so the nesting has almost nothing to do - but it is
// the same shelf-packer the other tools use, and the preview draws through it,
// so what is on screen is what lands in the file. Keeping it means a second
// panel could be added one day without the preview and the exporter drifting
...
// border line is a line, the name in Poppins is a fill, and one bag tag can
// carry both.
//
...
export function toSvg(panels, opts = {}) {
  const o = { title: 'Keychain', gap: 6, sheetWidth: 600, strokeWidth: 0.1, ...opts };
...
export function toPdf(panels, opts = {}) {
  ...
  ops.push('0 0 0 rg');
  // Every ring is closed into ONE path, and that path is filled once at the end.
  // Non-zero matches the winding the glyph data already carries, so a counter
  // stays a counter - but only across subpaths of the same path. Filling each
  // ring as it was drawn, the way this used to, gave the counter nothing to
  // cancel against and painted the middle of every A, B and R solid black.
  let filling = false;
  ...
  const objects = [
    ...
    `<< /Title (${pdfString(o.title)}) /Creator (SifuLaser Keychain Generator) >>`,
  ];
