// 2D face editor. Draws the live panel outline plus the decorations on it and lets
// you drag / scale / rotate them. Panel space is millimetres, y-up; the SVG is
// y-down, so every point goes through toView().
...
export class View2D {
  ...
  render(panel, objects, selection, opts = {}) {
    const samePanel = this.panel && this.panel.id === panel.id
  ...
  drawDims() {
    ...
    const px = 1 / this.zoom;
    const g = el('g', { class: 'dim-layer', fill: 'var(--dim)', stroke: 'var(--dim)' });
    const off = 10 * px;
